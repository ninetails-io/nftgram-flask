...
class SignupResource(Resource):
    def post(self):
        ...
        values = (user_id, password_hash, "user", dt)
        query = "INSERT INTO users(user_id, password, roles, date_joined) VALUES (?,?,?,?)"
...
class LoginResource(Resource):
    def post(self):
        ...
        if check_password_hash(user['password'], password):
            ...
            jwt_token = encode_auth_token(user_id).decode("utf-8")
            resp = make_response({"token": jwt_token}, 200)
            resp.headers["Authorization"] = "Bearer " + jwt_token
            return resp
        else:
            get_db().close()
            return error_response("NOT_FOUND", "The username and password combination provided was not found", 404)
